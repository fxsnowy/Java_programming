...
public class BigOhNotation {
	 ...
	 * of data (Bubble Sort) Algorithms with more nested iterations will result
	 * in O(N^3), O(N^4), etc. performance. Each pass through the outer loop
	 * (0)N requires us to go through the entire list again so N multiplies
	 ...
	 * each time through the algorithm. The Binary search is a perfect example
	 * of this. Pretty fast because the log N increases at a dramatically slower
	 * rate as N increases. O (log N) algorithms are very efficient because
	 * increasing the amount of data has little effect at some point early on
	 * because the amount of data is halved on each run through
	 */
	 ...
	 * number of comparisons we need to make with the Quick Sort we first know
	 * that it is comparing and moving values very efficiently without shifting.
	 * That means values are compared only once. So each comparison will reduce
	 * the possible final sorted lists in half.
	 * 
	 * Comparisons = log n! (Factorial of n) Comparisons = log n + log(n-1) +
	 * .... + log(1)
	 * 
	...
	public static void main(String[] args) {
		...
		BigOhNotation testAlgo4 = new BigOhNotation(30000);
